...
if __name__ == '__main__':
    try:
        ...
        while not rospy.is_shutdown():
            j = 0
            default_angle = [0, 0, 0]
            RPY_angle_data_old = rospy.get_param('RPY_angle_old', default_angle)
            RPY_angle_data_new_traj = rospy.get_param('RPY_angle_new', default_angle)
            #rospy.info(RPY_angle_data_old)
            #rospy.info(RPY_angle_data_new_traj)
            if RPY_angle_data_old != RPY_angle_data_new_traj:
                rate, cycle_gait_np_data = rpy.rpy_gait(RPY_angle_data_old, RPY_angle_data_new_traj)
                ...
                while (j<1):
                    for i in range(data_length):
                        ...
                        pause.sleep()
                    j = j + 1
                rospy.set_param('RPY_angle_old', RPY_angle_data_new_traj)
